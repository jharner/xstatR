...
/* added by Jun Tan to pop-up menu when right click or ctrl click */
extern int popupMenu(XEvent);
...
static char *GraphFontName = "7x13"; /* changed by Jun Tan */
     ...
     int modal;
{
  ...
  if (objectp(object)) {
    ...
    case KeyPress:    
      ...
      break;
    case ButtonPress:
      ctrFlag = (report.xbutton.state & (XK_Control_L | XK_Control_R));
      if(report.xbutton.button == 3 || 
	 (report.xbutton.button == 1 && ctrFlag)){
	if(!popupMenu(report))
	  do_button(report, dpy, win, object);
      }
      else
     ...
     int is_GW;
{
  ...
  /* Commented out by Jun Tan. close button is provided by most X Window
     Managers. menu button is replace by right click of the mouse button 
