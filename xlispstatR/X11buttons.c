...
/* The popupMenu is used to pop up menu when the right button is 
   pressed. It was put here simply because I reused the code for
   menu button which depend on a couple of global variable defined 
   in this file  -- Jun Tan */
int popupMenu(XEvent report){
  Display *dpy = StX11Display();
  int screen = StX11Screen();
  Window win, child;
  int x, y, item;
  ...
  win = report.xany.window;
  if (XFindContext(dpy, win, ObjectContext, (caddr_t *) &object) != 0 
      || !objectp(object) || GETWINDOWADDRESS(object) == NullWindow)
    return;
  ...
  if(menu == NIL)
    ...
    //draw_special_button(dpy, win, type, TRUE);
    XSetWindowBorderWidth(dpy, report.xany.window,
    			  button_border_width);
    item = StMObPopup(menu, x, y, NIL);
    //draw_special_button(dpy, win, type, FALSE);
    if (item > 0) send_message1(menu, sk_select, item);
